...
zend_op_array *phpdbg_compile_file(zend_file_handle *file, int type) {
	...
	dataptr = zend_hash_find_ptr(&PHPDBG_G(file_sources), ret->filename);
	if (dataptr != NULL) {
		// This file was previously compiled. Destroy the old source.
		destroy_op_array(&dataptr->op_array);
